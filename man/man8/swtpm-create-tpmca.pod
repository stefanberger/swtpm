...
B<swtpm-create-tpmca> is a tool to create a TPM 2-based CA that can be used by
B<swtpm_localca> to sign EK and platform certificates. It makes use of the Intel
pkcs11 driver and its tool (tpm2_ptool).
...
Start tpm2-abrmd first and then create the TPM key and TPM CA certificate:
...
To test the above TPM CA, run the following command:
...
B<swtpm_localca>, B<swtpm-localca.conf>
