...
Print all profiles found in the local an disto profiles directories and
the built-in ones. The names of the profiles read from directories will show
the name under which they can be loaded with the I<--profile-name> option
even though they are 'custom' profiles.
