...
=item B<--ek1keyalgo <key algorithm>> (since v0.11)

Choose the key algorithm for the 1st endorsement key. This key algorithm
...
ecc_nist_p521 or secp521r1, rsa2048, rsa3072, rsa4096, ml-kem-512,
ml-kem-768, ml-kem-1024, ml-dsa-44, ml-dsa-65, and ml-dsa-87.

Note that rsa4096, ML-KEM, and ML-DSA keys require the default-v2 or a later
profile.

If this option is used then the B<--rsa-keysize> option is ignored.

It is possible to choose a default 1st endorsement key algorithm with an
I<ek1keyalgo> entry in the I<swtpm_setup.conf> configuration file.

=item B<--ek2keyalgo <key algorithm>> (since v0.11)

Choose the key algorithm for the 2nd endorsement key. If the 1st and 2nd
endorsement key are of the same type then only one endorsement key will be
created. The default is secp384r1.

For valid values see the description of the B<--ek1keyalgo> option.
...
Valid values are: ecc_nist_p256 or secp256r1, ecc_nist_p384 or secp384r1,
ecc_nist_p521 or secp521r1, rsa2048, rsa3072, rsa4096, ml-dsa-44, ml-dsa-65,
and ml-dsa-87.

Note that rsa4096 and ML-DSA keys require the default-v2 or a later
profile.
...
=item B<cmdarg-ek1keyalgo> (since v0.11)

The I<--ek1keyalgo> option is supported.
