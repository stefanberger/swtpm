...
#define NEWKEY_ALGO   "rsa:3072"
#define CERT_HASH     "-sha256"
#define CERT_DAYS     "36500"
 ...
 */
static int run_openssl(const gchar **cmd, const gchar **env, const char *msg)
{
                               ...
                               const gchar *issuercert)
{
    ...
    if (access(signkey, R_OK) != 0 || access(issuercert, R_OK) != 0) {
        ...
        g_autofree gchar *openssl = g_find_program_in_path(OPENSSL_TOOL);
        ...
        cmd = concat_arrays(NULL,
                            (const gchar *[]) {
                                ...
                                swtpm_rootca_password == NULL ? "-noenc" : NULL,
                                NULL
                            }, FALSE);
        ...
        cmd = concat_arrays(NULL,
                            (const gchar *[]) {
                                ...
                                "-addext", "basicConstraints=critical,CA:TRUE",
                                "-addext", "keyUsage=critical,keyCertSign",
                                signkey_password == NULL ? "-noenc" : NULL,
                                NULL
                            }, FALSE);
...
/* Extract the public key parameters from a string like pubkey=1a3cd2,algo=ml-kem-1024.
                                      ...
                                      gchar **keyalgo)
{
    ...
    if (regcomp(&preg, "^pubkey=([0-9A-Fa-f]+),algo=([^,]+)$",
                REG_EXTENDED) != 0) {
        logerr(gl_LOGFILE, "Internal error: Could not compile regex\n");
        return 1;
    }
    ...
    if (regexec(&preg, key_params, 3, pmatch, 0) == 0) {
        *pubkey = g_strndup(&key_params[pmatch[1].rm_so],
                            pmatch[1].rm_eo - pmatch[1].rm_so);
        *keyalgo = g_strndup(&key_params[pmatch[2].rm_so],
                             pmatch[2].rm_eo - pmatch[2].rm_so);
        ret = 2;
    }
                       ...
                       const gchar *lockfile, const gchar *optsfile)
{
    ...
    switch (extract_pubkey_params(key_params, &pubkey, &keyalgo)) {
    case 1:
        goto error;
    case 2: /* have pubkey and keyalgo */
...
static void usage(const char *prgname)
{
   printf(
        ...
        "--ek key-param        The modulus of an RSA key or x=...,y=,... for EC keys\n"
        "                      or pubkey=...,algo=... for ML-DSA and ML-KEM keys\n"
